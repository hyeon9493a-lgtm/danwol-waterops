...
# -------------------------------------------------------------
# 8. 안전·보건 교육 실시일지 및 안내 AI 자동작성 & 교안 추출 보관 모듈
# -------------------------------------------------------------
if menu == "📋 8. 안전·보건 교육 실시일지 및 안내 AI 자동작성 & 월별보관":
    st.title("📋 단월처리시설 안전·보건 교육 실시일지 & 안내 자동작성기")
    st.caption("🔒 7·8월 공인 양식 1:1 직결 · 결재라인(담당/결재) 전자서명 · 교안 텍스트 AI 자동 추출 & 요약 보관 · 내부직원 5인 명단 탑재")
...
3. 기상청 폭염특보 단계별 작업관리 및 밀폐공간(맨홀, 정화조 등) 동시 질식재해 예방수칙 주지"""
        },
...
3. 약품 누출 시 중화제 살포 요령 및 세안·세척기 작동 점검"""
        ...
        "근골격계질환 예방 및 올바른 중량물 운반작업": {
            "type": "일반 안전보건교육 (매반기 12시간이상)",
            "hours": "09:00 ~ 09:30 (30분간)",
            "place": "단월공공하수처리시설 사무실",
            "instructor": "주영규 시설장",
            "content": """1. 중량물 취급 시 허리를 펴고 다리를 굽혀 신체 밀착 운반 원칙 준수
2. 20kg 초과 설비 부품 및 약품 포대 인양 시 2인 1조 또는 호이스트 활용
3. 작업 전·후 관절 및 근육 피로 회복을 위한 안전 스트레칭 실시"""
        }
    }
        ...
        with col_e1:
            ...
            if uploaded_edu_files:
                for up_f in uploaded_edu_files:
                    ...
                    if "폭염" in extracted_raw or "온열" in extracted_raw or "열사병" in extracted_raw:
...
3. 체감온도 35℃ 이상 시 오후 2~5시 옥외작업 단축 및 밀폐공간(정화조 등) 동시 질식위험 점검"""
                    elif "근골격" in extracted_raw:
                        detected_subject = "근골격계질환 예방 및 올바른 중량물 운반작업"
                        extracted_summary = """1. 부적절한 자세 및 반복작업에 따른 근골격계 부담작업 예방
2. 중량물 인양 시 2인 1조 작업 및 보조 운반구(호이스트) 적극 사용
3. 작업 전·후 스트레칭 및 휴게시간 준수"""
...
3. 공기호흡기 및 송기마스크 착용 철저"""
                ...
                if extracted_summary:
                    ...
                    if st.button("⚡ [추출된 교안 내용으로 교육양식 자동 채우기]", type="primary"):
            ...
            default_subj_choice = st.session_state.get("auto_filled_subj", list(edu_subject_db.keys())[0])
            ...
            with col_sub2:
                ...
                edu_special_note = st.text_input("특기 사항", value="해당사항 없음 (전원 참석 및 현장 교육 완료)")

            edu_content = st.text_area("교육 내용 (AI 자동생성 / 직접 수정 가능)", value=def_content, height=130)
        ...
        with col_e2:
            ...
            with col_pad1:
                ...
                canvas_writer = st_canvas(stroke_width=2, stroke_color="#000000", background_color="#F8F9FA", height=80, width=170, drawing_mode="freedraw", key="canvas_edu_writer_v350")
            with col_pad2:
                st.caption("✍️ **결재자(시설장) 서명**")
                canvas_approver = st_canvas(stroke_width=2, stroke_color="#000000", background_color="#F8F9FA", height=80, width=170, drawing_mode="freedraw", key="canvas_edu_approver_v350")
            ...
            for num, d_dept, d_name, d_sign in default_staff:
                ...
                with col_st2: s_dept = st.text_input(f"소속 #{num}", value=d_dept, key=f"edu_dept_{num}_v350", label_visibility="collapsed")
                with col_st3: s_name = st.text_input(f"성명 #{num}", value=d_name, key=f"edu_name_{num}_v350", label_visibility="collapsed")
    ...
    with tab_edu_archive:
        ...
        if all_month_dirs:
            sel_edu_m_dir = st.selectbox("📅 보관 월 선택", all_month_dirs, key="sel_edu_arch_m_v350")
            ...
            with col_vf1:
                sel_f_view = st.selectbox("열람 및 다운로드할 파일 선택", files_in_m, key="sel_edu_file_to_view_v350")
            with col_vf2:
                st.write(""); st.write("")
                if st.button("🗑️ 선택 파일 삭제", type="secondary", use_container_width=True, key="btn_del_edu_file_v350"):
