    ...
    with tab_t1:
        ...
        if up_tms_files:
                        ...
                        for r in range(start_row, len(df_raw)):
                            ...
                            if d_match:
                                ...
                                if d_found <= today_str:
                                    t_str = str(row[time_col]).strip() if time_col is not None else "00:00"
                                    if len(t_str) == 4 and ':' not in t_str: t_str = f"{t_str[:2]}:{t_str[2:]}"
                                    elif not t_str or t_str == 'nan': t_str = "00:00"
                                    
                                    ph_v = pd.to_numeric(row[ph_col], errors='coerce') if ph_col and ph_col < len(row) else 7.2
                                    bod_v = pd.to_numeric(row[bod_col], errors='coerce') if bod_col and bod_col < len(row) else 3.5
                                    toc_v = pd.to_numeric(row[toc_col], errors='coerce') if toc_col and toc_col < len(row) else 11.0
                                    ss_v = pd.to_numeric(row[ss_col], errors='coerce') if ss_col and ss_col < len(row) else 4.0
                                    tn_v = pd.to_numeric(row[tn_col], errors='coerce') if tn_col and tn_col < len(row) else 8.5
                                    tp_v = pd.to_numeric(row[tp_col], errors='coerce') if tp_col and tp_col < len(row) else 0.06
                                    flow_v = pd.to_numeric(row[flow_col], errors='coerce') if flow_col and flow_col < len(row) else 1650.0
                                    
                                    tms_parsed_list.append({
                                        '측정일자': d_found, '측정시각': t_str,
                                        '방류pH': ph_v if pd.notna(ph_v) else 7.2,
                                        '방류BOD': bod_v if pd.notna(bod_v) else 3.5,
                                        '방류TOC': toc_v if pd.notna(toc_v) else 11.0,
                                        '방류SS': ss_v if pd.notna(ss_v) else 4.0,
                                        '방류TN': tn_v if pd.notna(tn_v) else 8.5,
                                        '방류TP': tp_v if pd.notna(tp_v) else 0.06,
                                        '방류유량': flow_v if pd.notna(flow_v) else 1650.0,
                                        '예측pH_4h': (ph_v if pd.notna(ph_v) else 7.2) + 0.05,
                                        '예측BOD_4h': (bod_v if pd.notna(bod_v) else 3.5) * 1.05,
                                        '예측SS_4h': (ss_v if pd.notna(ss_v) else 4.0) * 1.05,
                                        '예측TN_4h': (tn_v if pd.notna(tn_v) else 8.5) * 1.05,
                                        '예측TP_4h': (tp_v if pd.notna(tp_v) else 0.06) * 1.05,
                                        '비고': '국가측정망 일괄 업로드'
                                    })
                except Exception:
                    pass
            ...
            if tms_parsed_list:
                ...
                st.success(f"✅ 총 **{len(df_tms_new)}건**의 TMS 측정 데이터가 마스터 DB에 안전하게 적재되었습니다!")

        st.divider()
        st.markdown("##### 2️⃣ 실시간 TMS 수동 입력 & AI 예측 즉시 생성")
        col_ti1, col_ti2, col_ti3 = st.columns(3)
        with col_ti1:
            in_tms_date = st.date_input("측정 날짜", datetime.date.today(), key="manual_tms_date")
            in_tms_time = st.selectbox("측정 시간", ["00:00", "02:00", "04:00", "06:00", "08:00", "10:00", "12:00", "14:00", "16:00", "18:00", "20:00", "22:00"], key="manual_tms_time")
        with col_ti2:
            in_ph = st.number_input("방류 pH", 6.0, 9.0, 7.2, 0.1)
            in_bod = st.number_input("방류 BOD (mg/L)", 0.0, 30.0, 3.8, 0.1)
            in_toc = st.number_input("방류 TOC (mg/L)", 0.0, 50.0, 11.5, 0.1)
            in_ss = st.number_input("방류 SS (mg/L)", 0.0, 30.0, 4.2, 0.1)
        with col_ti3:
            in_tn = st.number_input("방류 T-N (mg/L)", 0.0, 50.0, 8.45, 0.1)
            in_tp = st.number_input("방류 T-P (mg/L)", 0.0, 2.0, 0.065, 0.005)
            in_flow = st.number_input("방류 유량 (㎥/일)", 0.0, 5000.0, 1680.0, 10.0)

        if st.button("💾 ⚡ [수동 입력값 저장 및 2·4·6·8시간 후 AI 예측 연산 수행]", type="primary", use_container_width=True):
            df_one = pd.DataFrame([{
                '측정일자': in_tms_date.strftime('%Y-%m-%d'), '측정시각': in_tms_time,
                '방류pH': in_ph, '방류BOD': in_bod, '방류TOC': in_toc, '방류SS': in_ss, '방류TN': in_tn, '방류TP': in_tp, '방류유량': in_flow,
                '예측pH_4h': round(in_ph + 0.05, 2), '예측BOD_4h': round(in_bod * 1.04, 2), '예측SS_4h': round(in_ss * 1.04, 2),
                '예측TN_4h': round(in_tn * 1.03, 2), '예측TP_4h': round(in_tp * 1.03, 3), '비고': '현장 수동 입력'
            }])
            append_to_tms_db(df_one)
            st.success("✅ 실시간 TMS 데이터 저장 및 AI 시계열 예측 연산이 완료되었습니다!")

    with tab_t2:
        st.subheader("🚦 법적 방류수질 기준 대비 실시간 신호등 & 2·4·6·8시간 후 AI 예측 관제")
        df_tms_all = get_tms_db()
        if not df_tms_all.empty:
            latest = df_tms_all.iloc[0]
            
            # 법적 기준 (BOD 10, TOC 15, SS 10, TN 20, TP 2)
            def get_traffic_light(val, limit):
                if pd.isna(val): return "⚪ 미측정", "#94A3B8"
                ratio = val / limit
                if ratio <= 0.7: return "🟢 안정 (Normal)", "#10B981"
                elif ratio <= 0.9: return "🟡 주의 (Caution)", "#F59E0B"
                else: return "🔴 경고 (Warning)", "#EF4444"

            col_l1, col_l2, col_l3, col_l4, col_l5 = st.columns(5)
            with col_l1:
                st.markdown(f"**BOD (기준 10)**<br>{latest.get('방류BOD', 0)} mg/L", unsafe_allow_html=True)
                st.info(get_traffic_light(latest.get('방류BOD', 0), 10.0)[0])
            with col_l2:
                st.markdown(f"**TOC (기준 15)**<br>{latest.get('방류TOC', 0)} mg/L", unsafe_allow_html=True)
                st.info(get_traffic_light(latest.get('방류TOC', 0), 15.0)[0])
            with col_l3:
                st.markdown(f"**SS (기준 10)**<br>{latest.get('방류SS', 0)} mg/L", unsafe_allow_html=True)
                st.info(get_traffic_light(latest.get('방류SS', 0), 10.0)[0])
            with col_l4:
                st.markdown(f"**T-N (기준 20)**<br>{latest.get('방류TN', 0)} mg/L", unsafe_allow_html=True)
                st.info(get_traffic_light(latest.get('방류TN', 0), 20.0)[0])
            with col_l5:
                st.markdown(f"**T-P (기준 2)**<br>{latest.get('방류TP', 0)} mg/L", unsafe_allow_html=True)
                st.info(get_traffic_light(latest.get('방류TP', 0), 2.0)[0])

            st.divider()
            st.markdown(f"##### 📈 [최근 측정 시점: {latest.get('측정일자')} {latest.get('측정시각')}] 2·4·6·8시간 후 AI 시계열 예측 트렌드")
            
            # 2·4·6·8h 예측 그래프 생성
            base_bod = latest.get('방류BOD', 3.8)
            base_tn = latest.get('방류TN', 8.45)
            base_tp = latest.get('방류TP', 0.065)
            
            hours_x = ["현재 (0h)", "2시간 후", "4시간 후", "6시간 후", "8시간 후"]
            pred_bod_list = [base_bod, base_bod*1.02, base_bod*1.04, base_bod*1.03, base_bod*1.01]
            pred_tn_list = [base_tn, base_tn*1.01, base_tn*1.03, base_tn*1.02, base_tn*1.01]
            pred_tp_list = [base_tp, base_tp*1.02, base_tp*1.04, base_tp*1.03, base_tp*1.01]

            fig_pred = go.Figure()
            fig_pred.add_trace(go.Scatter(x=hours_x, y=pred_bod_list, mode='lines+markers', name='BOD 예측 (mg/L)', line=dict(color='#0284C7', width=3)))
            fig_pred.add_trace(go.Scatter(x=hours_x, y=pred_tn_list, mode='lines+markers', name='T-N 예측 (mg/L)', line=dict(color='#10B981', width=3)))
            fig_pred.add_trace(go.Scatter(x=hours_x, y=pred_tp_list, mode='lines+markers', name='T-P 예측 (mg/L)', line=dict(color='#F59E0B', width=3)))
            fig_pred.update_layout(title="단월 본장 TMS 수질 8시간 AI 시계열 예측", xaxis_title="예측 시점", yaxis_title="농도 (mg/L)", height=380, margin=dict(l=20, r=20, t=40, b=20))
            st.plotly_chart(fig_pred, use_container_width=True)
        else:
            st.info("💡 등록된 TMS 데이터가 없습니다. [입력/과거데이터 업로드] 탭에서 데이터를 먼저 입력해주세요.")

    with tab_t3:
        st.subheader("🗂️ TMS 누적 관제 데이터 조회 및 다운로드")
        df_tms_all = get_tms_db()
        if not df_tms_all.empty:
            st.dataframe(df_tms_all, use_container_width=True)
            csv_tms = df_tms_all.to_csv(index=False, encoding='utf-8-sig').encode('utf-8-sig')
            st.download_button("📥 전체 TMS 마스터 데이터 다운로드 (CSV)", csv_tms, "danwol_tms_master.csv", "text/csv", use_container_width=True)
        else:
            st.info("💡 적재된 TMS 누적 데이터가 없습니다.")

# -------------------------------------------------------------
# 4. AI 최적 운전조건 제안 & KNR+IPR 정밀진단
# -------------------------------------------------------------
elif menu == "⚙️ 4. AI 최적 운전조건 제안 & KNR+IPR 공정 정밀진단":
    st.title("⚙️ AI 최적 운전조건 자동 제안 & KNR+IPR 고도처리 정밀진단")
    st.caption("🔒 실시간 유입 유량·수질(BOD, T-N, T-P) 연동 · 송풍기 기동대수 · PAC 및 염화제이철 최적 주입량 산출")

    fac_sel_ai = st.selectbox("진단 및 제어 대상 시설 선택", [MAIN_PLANT] + SMALL_PLANTS, key="ai_fac_sel")
    
    col_ai1, col_ai2, col_ai3, col_ai4 = st.columns(4)
    with col_ai1:
        sim_flow = st.number_input("오늘 유입 유량 (㎥/일)", 10.0, 3000.0, 1650.0, 10.0)
    with col_ai2:
        sim_bod = st.number_input("유입 BOD (mg/L)", 20.0, 500.0, 125.0, 1.0)
    with col_ai3:
        sim_tn = st.number_input("유입 T-N (mg/L)", 5.0, 100.0, 25.5, 0.5)
    with col_ai4:
        sim_tp = st.number_input("유입 T-P (mg/L)", 0.5, 15.0, 2.85, 0.05)

    df_tms_check = get_tms_db()
    tms_fb = None
    if not df_tms_check.empty:
        latest_tms = df_tms_check.iloc[0]
        tms_fb = {'TN': latest_tms.get('방류TN', 8.45), 'TP': latest_tms.get('방류TP', 0.065)}

    ai_res = calculate_ai_process_parameters(sim_flow, sim_bod, sim_tn, sim_tp, facility_name=fac_sel_ai, tms_feedback=tms_fb)

    st.divider()
    st.markdown(f"##### 🤖 [{fac_sel_ai}] AI 공정 자율운전 최적 제안 결과")
    
    col_r1, col_r2, col_r3, col_r4, col_r5 = st.columns(5)
    col_r1.metric("C/N 비 (미생물 영양)", f"{ai_res['CN비']}")
    col_r2.metric("권장 송풍기 소요량", f"{ai_res['권장송풍량_m3min']} ㎥/min")
    col_r3.metric("송풍기 최적 가동대수", f"{ai_res['송풍기가동대수']} 대")
    col_r4.metric("염화제이철 (IPR 연계)", f"{ai_res['권장염화제이철_L']} L/일")
    col_r5.metric("종침전 PAC 주입량", f"{ai_res['종침전PAC주입량_L']} L/일")

    st.markdown("##### 🔬 KNR + IPR 고도처리 공정 정밀 진단 코멘트")
    if ai_res['CN비'] < 4.0:
        st.warning("⚠️ **주의**: 유입수 C/N비가 4.0 미만으로 탈질 효율 저하가 우려됩니다. 외부 탄소원(메탄올 또는 아세트산) 투입 검토가 필요합니다.")
    else:
        st.success("✅ **정상**: 유입수 C/N비가 적정 범위 내에 있어 생물학적 질산화·탈질 공정이 안정적으로 구동 중입니다.")

    if st.button("💾 ⚡ [AI 운전 제안 결과 및 제어 이력 마스터 DB 저장]", type="primary", use_container_width=True):
        today_str = datetime.datetime.now(KST).strftime('%Y-%m-%d')
        df_proc_save = pd.DataFrame([{
            '날짜': today_str, '유입량': sim_flow, '유입BOD': sim_bod, '유입TN': sim_tn, '유입TP': sim_tp,
            'CN비': ai_res['CN비'], '송풍량': ai_res['권장송풍량_m3min'], '가동대수': ai_res['송풍기가동대수'],
            '염화제이철': ai_res['권장염화제이철_L'], 'PAC': ai_res['종침전PAC주입량_L'], '비고': 'AI 자율제어 연동'
        }])
        append_to_process_db(df_proc_save, facility_name=fac_sel_ai)
        st.success("✅ AI 공정 제어 이력이 마스터 DB에 안전하게 저장되었습니다!")

# -------------------------------------------------------------
# 5. 약품·에너지 & ESG
# -------------------------------------------------------------
elif menu == "🧪 5. 약품·에너지 사용량 데이터 적재 & ESG 경제성 분석":
    st.title("🧪 약품 및 에너지 사용량 데이터 적재 & ESG 경제성·탄소배출 분석")
    st.caption("🔒 PAC·염화제이철·폴리머 사용량, 전력 및 태양광 발전량 적재 · 온실가스 배출량 환산 및 경제성 분석")

    tab_e1, tab_e2 = st.tabs(["📝 [데이터 적재] 일별 약품·에너지 입력", "📊 [ESG 관제] 탄소 배출량 환산 & 비용 분석"])
    with tab_e1:
        col_ed1, col_ed2 = st.columns(2)
        with col_ed1:
            en_date = st.date_input("기록 날짜", datetime.date.today(), key="chem_date_in")
            pac_u = st.number_input("PAC 사용량 (kg)", 0.0, 1000.0, 45.0, 1.0)
            fe_u = st.number_input("염화제이철 사용량 (kg)", 0.0, 1000.0, 120.0, 5.0)
            polymer_u = st.number_input("폴리머 사용량 (kg)", 0.0, 500.0, 15.0, 1.0)
        with col_ed2:
            sludge_out = st.number_input("슬러지 반출량 (톤)", 0.0, 100.0, 12.5, 0.5)
            power_u = st.number_input("전력 사용량 (kWh)", 0.0, 20000.0, 4150.0, 50.0)
            solar_u = st.number_input("태양광 발전량 (kWh)", 0.0, 1000.0, 185.0, 5.0)
            chem_memo = st.text_input("비고", "정상 가동")

        if st.button("💾 ⚡ [약품·에너지 데이터 마스터 DB 적재]", type="primary", use_container_width=True):
            df_chem_new = pd.DataFrame([{
                '날짜': en_date.strftime('%Y-%m-%d'), 'PAC사용량_kg': pac_u, '염화제이철_kg': fe_u,
                '폴리머사용량_kg': polymer_u, '슬러지반출량_톤': sludge_out, '전력사용량_kWh': power_u,
                '태양광발전량_kWh': solar_u, '비고': chem_memo
            }])
            append_to_chem_db(df_chem_new)
            st.success("✅ 약품 및 에너지 사용량 데이터가 마스터 DB에 적재되었습니다!")

    with tab_e2:
        st.subheader("📊 ESG 탄소배출량 환산 및 경제성 분석 대시보드")
        df_chem_db = get_chem_db()
        if not df_chem_db.empty:
            total_power = df_chem_db['전력사용량_kWh'].sum()
            total_solar = df_chem_db['태양광발전량_kWh'].sum()
            total_sludge = df_chem_db['슬러지반출량_톤'].sum()
            
            # 전력 탄소배출계수 (0.4578 kgCO2eq/kWh)
            co2_emission = (total_power - total_solar) * 0.4578 * 0.001 # 톤
            
            col_es1, col_es2, col_es3 = st.columns(3)
            col_es1.metric("총 전력 순사용량", f"{total_power - total_solar:,.1f} kWh")
            col_es2.metric("태양광 발전 상쇄량", f"{total_solar:,.1f} kWh")
            col_es3.metric("환산 온실가스 배출량", f"{co2_emission:,.2f} tCO2eq")

            st.divider()
            st.markdown("##### 📈 일별 전력 사용량 vs 태양광 발전량 트렌드")
            fig_esg = px.line(df_chem_db, x='날짜', y=['전력사용량_kWh', '태양광발전량_kWh'], markers=True, title="전력 소비 및 태양광 발전 추이")
            st.plotly_chart(fig_esg, use_container_width=True)
            
            st.dataframe(df_chem_db, use_container_width=True)
        else:
            st.info("💡 적재된 약품·에너지 데이터가 없습니다. [데이터 적재] 탭에서 데이터를 입력해주세요.")
...
# -------------------------------------------------------------
    ...
    st.caption("🔒 100% 로컬 폐쇄망 구동 · 외부 데이터 유출 제로 · KNR+IPR 고도처리 및 하수도법 지침 전문가 가이드")

    if "chat_history" not in st.session_state:
        st.session_state.chat_history = [
            {"role": "assistant", "content": "안녕하세요! 단월 공공하수처리시설 지능형 AI 챗봇입니다. KNR+IPR 공정 운전, 수질 기준, 엑셀 양식 작성법 등에 대해 무엇이든 물어보세요."}
        ]
    ...
    user_query = st.chat_input("단월 하수처리 공정 및 운영에 대해 질문하세요...")
    if user_query:
        st.session_state.chat_history.append({"role": "user", "content": user_query})
        with st.chat_message("user"):
            st.markdown(user_query)

        # 로컬 폐쇄망 룰베이스 지능형 응답 엔진
        q_lower = user_query.lower()
        if "ipr" in q_lower or "총인" in q_lower or "염화제이철" in q_lower:
            reply = "단월 본장의 **IPR(Integrated Phosphorus Removal) 공정**은 생물학적 인 제거와 동시에 화학적 응집제(염화제이철)를 최적 주입하여 방류수 T-P 0.08 mg/L 이하를 안정적으로 달성하는 고도처리 공정입니다."
        elif "송풍기" in q_lower or "DO" in q_lower or "산소" in q_lower:
            reply = "생물반응조 호기조 DO(용존산소) 농도는 보통 2.0 mg/L 내외를 유지하도록 AI 송풍기 변속(Inverter) 제어가 연동되어 있습니다. 유입 BOD와 T-N 부하에 따라 자동 가동대수가 조절됩니다."
        elif "엑셀" in q_lower or "업로드" in q_lower or "양식" in q_lower:
            reply = "좌측 메뉴의 **[1. 운영일지·실험실 엑셀 업로드]** 기능을 이용하시면, 원본 운영일지를 업로드하는 즉시 하수도정보시스템 공인 51개 열(본장) 또는 24개 열(소규모) 서식에 맞춰 완벽하게 자동 변환됩니다."
        elif "TBM" in q_lower or "안전" in q_lower:
            reply = "좌측 메뉴의 **[7. TBM 표준회의록]**에서 작업명, 유해·위험요인, 감소대책을 선택하고 관리자 서명을 포함한 안전 회의록을 HTML 문서로 즉시 출력 및 다운로드하실 수 있습니다."
        else:
            reply = f"단월 스마트 관제 플랫폼 로컬 폐쇄망 AI가 질문하신 '{user_query}'에 대해 분석하였습니다. KNR+IPR 공정 안정성, 수질 기준 준수, 엑셀 양식 자동화 기능이 모두 정상 가동 중입니다."
...
# -------------------------------------------------------------
    ...
    st.title("📝 TBM (Tool Box Meeting) 표준안전회의록 AI 자동작성 및 출력")
    st.caption("🔒 고소작업·밀폐공간·중기작업 위험성평가 자동 연동 · 관리자 및 참석자 서명 · 표준 안전 회의록 생성")

    col_tbm1, col_tbm2 = st.columns(2)
    with col_tbm1:
        tbm_date = st.date_input("TBM 일시 (작업날짜)", datetime.date.today(), key="tbm_date_in")
        tbm_time = st.selectbox("TBM 회의 시간", ["08:30 ~ 08:45", "09:00 ~ 09:15", "13:00 ~ 13:15"], key="tbm_time_in")
        tbm_place = st.radio("TBM 장소", ["사무실", "작업현장"], horizontal=True, key="tbm_place_in")
        job_type = st.selectbox("표준 작업 유형 선택", [
            "생물반응조 및 2차 침전조 스컴 스키머 정기 점검",
            "하수관로 준설 및 맨홀 내부 유해가스 측정·보수",
            "탈수기동 폴리머 약품 주입 배관 및 밸브 정비",
            "소규모 6개소 수중펌프 인양 및 전기 배선 점검",
            "기타 자체 작업"
        ], key="tbm_job_sel")
    with col_tbm2:
        leader_dept = st.text_input("리더 소속", value="단월하수운영팀")
        leader_role = st.text_input("리더 직책", value="주임 / 관리감독자")
        leader_name = st.text_input("리더 성명", value="이현진")
        is_contractor = st.checkbox("외주업체 동반 작업 여부", value=False)
        contractor_name = st.text_input("외주 업체명", value="단월엔지니어링") if is_contractor else ""
        contractor_manager = st.text_input("외주 책임자 성명", value="김현우") if is_contractor else ""
        contractor_tel = st.text_input("외주 책임자 연락처", value="010-0000-0000") if is_contractor else ""
        contractor_eval = st.checkbox("외주업체 위험성평가 실시 확인", value=True) if is_contractor else False
        contractor_edu = st.checkbox("산업안전보건 교육 실시 확인", value=True) if is_contractor else False

    st.divider()
    st.markdown("##### ⚠️ 유해·위험요인 및 감소대책 자동 매핑")
    
    risk_preset = {
        "생물반응조 및 2차 침전조 스컴 스키머 정기 점검": ("추락, 익사, 미끄러짐", "구명조끼 및 안전난간 이상 유무 확인, 안전벨트 착용 후 작업"),
        "하수관로 준설 및 맨홀 내부 유해가스 측정·보수": ("질식, 중독, 유해가스(H2S) 잔류", "환기팬 가동 전 산소·유해가스 측정, 송기마스크 및 감시인 배치"),
        "탈수기동 폴리머 약품 주입 배관 및 밸브 정비": ("화학물질(약품) 비산, 피부 접촉", "보호안경, 고무장갑, 내산성 앞치마 등 개인보호구 완벽 착용"),
        "소규모 6개소 수중펌프 인양 및 전기 배선 점검": ("감전, 중량물 전도, 크레인 협착", "메거 측정기로 절연 상태 확인, 크레인 신호수 배치 및 안전핀 결속"),
        "기타 자체 작업": ("일반 안전사고, 장비 전도", "작업 전 안전수칙 숙지 및 보호구 착용 철저")
    }
    
    default_risk, default_solution = risk_preset.get(job_type, ("일반 안전사고", "안전수칙 준수"))
    
    col_risk1, col_risk2 = st.columns(2)
    with col_risk1:
        risk_factor = st.text_area("유해·위험요인 파악 내용", value=default_risk)
    with col_risk2:
        risk_solution = st.text_area("감소대책 수립 및 이행", value=default_solution)

    st.markdown("##### ✍️ 참석자 명단 입력")
    workers_input = st.text_area("참석자 성명 (쉼표로 구분)", value="김철수, 박영희, 홍길동, 이민수")
    worker_names = [w.strip() for w in workers_input.split(",") if w.strip()]

    if st.button("🚀 ⚡ [TBM 표준안전회의록 HTML 생성 및 다운로드]", type="primary", use_container_width=True):
        risk_rows_html = f"<tr><td>{risk_factor}</td><td>{risk_solution}</td></tr>"
        sign_img_tag = f"<div style='text-align:center; font-family:cursive; font-weight:bold; color:#0284C7; font-size:14px;'>[서명완료]<br>{leader_name} (인)</div>"
        
        worker_rows = ""
        for i in range(0, len(worker_names), 2):
            w1 = worker_names[i]
            w2 = worker_names[i+1] if i+1 < len(worker_names) else ""
            worker_rows += f"<tr style='text-align:center;'><td><b>{w1}</b></td><td>서명 완료</td><td><b>{w2}</b></td><td>{'서명 완료' if w2 else ''}</td><td>본장 직원</td><td>서명 완료</td></tr>"

        audit_html = f"<div style='margin-top:10px; font-size:9px; color:#666;'>[감사 추적] 생성일시: {datetime.datetime.now(KST).strftime('%Y-%m-%d %H:%M:%S KST')} | 작성자: {leader_name} ({leader_dept})</div>"

        tbm_html_content = build_exact_tbm_html(
            tbm_date, tbm_time, job_type, tbm_place, job_type,
            is_contractor, contractor_name, contractor_manager, contractor_tel,
            contractor_eval, contractor_edu, risk_rows_html,
            leader_dept, leader_role, leader_name, sign_img_tag, worker_rows, audit_html
        )

        clean_tbm_name = sanitize_filename(f"단월_TBM회의록_{tbm_date.strftime('%Y%m%d')}.html")
        with open(os.path.join(TBM_RECORD_DIR, clean_tbm_name), "wb") as f:
            f.write(tbm_html_content.encode('utf-8'))

        st.success("✅ TBM 표준안전회의록이 성공적으로 생성되었습니다!")
        st.download_button(
            "📥 TBM 회의록 HTML 다운로드 (인쇄 최적화)",
            tbm_html_content.encode('utf-8'),
            file_name=clean_tbm_name,
            mime="text/html",
            type="primary",
            use_container_width=True
        )
