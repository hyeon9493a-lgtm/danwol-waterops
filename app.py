...
# -------------------------------------------------------------
# 6. Q&A 챗봇 (전체 공정 제어 지식 엔진 전면 확장)
# -------------------------------------------------------------
elif menu == "🤖 6. 단월 AI 지능형 공정 Q&A 챗봇 (Gemini 연동)":
    st.title("🤖 단월 하수처리시설 AI 지능형 공정 도우미 (전 공정 지식 엔진 탑재)")
    ...
    def query_danwol_full_process_ai(user_query):
        ...
        # 1) 본장 KNR 공법 / 질소(T-N) 제어
        if any(k in q for k in ["knr", "질소", "t-n", "탈질", "질산화", "내부반송", "무산소"]):
            return (
                "💡 **[단월 본장 KNR 질소(T-N) 고도처리 제어 가이드]**\n\n"
                "1. **C/N 비 관리**: 안정적인 탈질을 위해 유입 수질의 **C/N 비(BOD/T-N)를 4.0 이상**으로 유지해야 합니다.\n"
                "2. **DO(용존산소) 제어**:\n"
                "   - **호기조 말단 DO**: 1.5 ~ 2.0 mg/L 유지 (과포기 시 무산소조로 산소가 유입되어 탈질 저해)\n"
                "   - **무산소조 DO**: 0.2 mg/L 이하 유지 (완전 무산소 환경 유지 필수)\n"
                "3. **내부 반송율(IPR 질산액 반송)**: 유입량 대비 **150% ~ 200%**로 운전하며, 동절기 수온 저하시 반송율을 10~15% 상향합니다.\n"
                "4. **동절기 수온 저하 시 대책**: 슬러지 일령(SRT)을 20~25일로 연장하여 질산화균의 유실을 방지하십시오."
            )
            
        # 2) 본장 IPR 및 3대 약품(염화제이철, PAC, 폴리머) 투입 제어
        elif any(k in q for k in ["ipr", "인", "t-p", "총인", "약품", "염화제이철", "염철", "pac", "응집제", "폴리머", "탈수"]):
            return (
                "💡 **[단월 본장 총인(T-P) 제거 및 3대 약품 투입 제어 수칙]**\n\n"
                "1. **IPR 공정 (급속혼화지) ➔ 염화제이철(FeCl3 38%) 선투입**:\n"
                "   - **목적**: 유입된 용존성 정인산염(PO4-P)을 화학적으로 불용화 침전 ($Fe^{3+} + PO_4^{3-} \rightarrow FePO_4 \downarrow$)\n"
                "   - **주입 기준**: P 대비 Fe 몰비 1.5 기준, 일평균 **약 25 ~ 30 L/일 (약 35~40 kg/일)** 자동 주입\n\n"
                "2. **2차 침전조 전단 (응집보조) ➔ PAC(17%) 후단 투입**:\n"
                "   - **목적**: 잔류 미세 콜로이드성 인 및 SS의 플록 조대화 및 침강성 촉진\n"
                "   - **주입 기준**: 일평균 **약 40 ~ 50 L/일 (약 45~55 kg/일)** 주입\n\n"
                "3. **탈수기동 (슬러지 탈수) ➔ 양이온 폴리머(Polymer) 주입**:\n"
                "   - **목적**: 원심탈수기 슬러지 케이크 형성 및 함수율 78% 이하 유지\n"
                "   - **주입 기준**: 건조 슬러지 톤당 2.0 ~ 3.0 kg (일평균 **약 2.0 kg/일**)"
            )
        ...
        # 3) 송풍기 및 전력(동력비) 제어
            ...
            return (
                ...
                "1. **실시간 AOR(실제 산소 요구량) 계산식**:\n"
                "   $$AOR = (Q \\times BOD \\times 1.2 + Q \\times T\\text{-}N \\times 4.57) \\times 10^{-3} \\text{ (kg } O_2/\\text{일)}$$\n"
                "2. **AI 송풍량 자동 제어**:\n"
                "   - 단월 본장 기준 AI 권장 풍량은 유입 부하에 따라 **12.5 ~ 14.5 ㎥/min** 수준입니다.\n"
                "   - 정용량(25 ㎥/min) 송풍기 1대를 인버터 50~60Hz 가변 제어하여 불필요한 과포기를 방지, **연간 약 18.2%의 동력비를 절감**합니다."
            )
        ...
        # 4) 삼가리 (SBR)
            ...
            return (
                ...
                "1. **공법 특성**: 단일 반응조에서 유입 ➔ 반응(호기/무산소) ➔ 침전 ➔ 배출 ➔ 휴지가 순차 진행되는 회분식 활성슬러지 공정입니다.\n"
                "2. **약품 사용**: **100% 무약품 생물학적 처리** 방식입니다.\n"
                "3. **운전 팁**:\n"
                "   - 질소(T-N) 농도 상승 시 유입 후 비포기(무산소) 교반 시간을 20분 연장하십시오.\n"
                "   - 침전 행정 중 디캔터 배출 시 슬러지 계면이 디캔터 흡입구에 닿지 않도록 배출 레벨을 사전 점검하십시오."
            )
        ...
        # 5) 산음 (SWPP)
            ...
            return (
                ...
                "1. **공법 특성**: 수중포기 침전일체형 고도처리 공정으로, 생물 반응과 침전이 일체형 수조에서 연속 수행됩니다.\n"
                "2. **운전 관리**:\n"
                "   - 수중 에어레이터의 흡입구 막힘 여부를 주 1회 점검하십시오.\n"
                "   - 무약품 시설이므로 적정 MLSS(2,500 ~ 3,500 mg/L)를 유지하고 잉여 슬러지를 정기 인발하십시오."
            )
        ...
        # 6) 진목 (고효율오수정화 + SOD)
            ...
            return (
                ...
                "1. **공법 특성**: 미생물 접촉산화 여재와 SOD(탈질 전용조)가 결합된 고효율 질소 제거 공정입니다.\n"
                "2. **운전 관리**:\n"
                "   - 접촉재 표면의 생물막 과다 부착 시 역세척 공정을 수동 가동하여 여재 폐색을 방지하십시오.\n"
                "   - SOD조의 혐기/무산소 환원 전위(ORP)가 -150 ~ -200 mV 범위에 있는지 확인하십시오."
            )
            
        # 7) 몰운 (IC-SBR)
        elif any(k in q for k in ["몰운", "ic-sbr", "icsbr"]):
            return (
                "💡 **[몰운 소규모 시설 (60 ㎥/일, IC-SBR) 공정 제어]**\n\n"
                "1. **공법 특성**: 간헐 포기식 회분 반응조로 반응조 내 **PAC 단독 직접 투입** 설비를 갖추고 있습니다.\n"
                "2. **약품 주입**: T-P 농도 0.2 mg/L 초과 위험 시 반응조 포기 행정 말기에 PAC을 일 **10 ~ 15 L** 주입하십시오."
            )
            
        # 8) 단월마을 & 당의 (IC-SBR 무약품)
        elif any(k in q for k in ["단월마을", "당의"]):
            return (
                "💡 **[단월마을(30 ㎥/일) & 당의(45 ㎥/일) IC-SBR 공정 제어]**\n\n"
                "1. **공법 특성**: 간헐 포기 회분식 공정으로 별도의 화학약품 없이 미생물 체류시간(SRT) 제어만으로 BOD와 T-N을 동시 제거합니다.\n"
                "2. **핵심 제어**: 포기 60분 / 비포기(교반) 60분의 간헐 사이클 타이머를 정기 보정하십시오."
            )
            
        # 9) 슬러지 팽화 및 침강성 불량 (SVI 이상)
        elif any(k in q for k in ["팽화", "bulking", "svi", "슬러지 부상", "거품", "스컴"]):
            return (
                "💡 **[슬러지 팽화(Bulking) 및 침강 불량 긴급 조치 매뉴얼]**\n\n"
                "1. **원인 진단**: 사상균성 팽화(DO 부족 또는 저부하 지속) vs 비사상균성 팽화(점성 다당류 축적)\n"
                "2. **즉각 조치 사항**:\n"
                "   - 호기조 DO를 일시적으로 2.5 ~ 3.0 mg/L로 상향하여 사상균 번식 억제\n"
                "   - 침전조 전단에 PAC 응집제를 평시대비 20% 증량 주입하여 플록 무게 형성 촉진\n"
                "   - 잉여 슬러지 인발량을 15% 늘려 슬러지 일령(SRT)을 15일 내외로 단축"
            )
            
        # 10) TMS 비상 경보 및 기준 초과 시
        elif any(k in q for k in ["tms", "초과", "경보", "비상", "방류수질기준"]):
            return (
                "💡 **[단월 본장 법적 방류수질 기준 및 TMS 비상 대응]**\n\n"
                "| 항목 | 법적 기준 | 비상 알람 기준 | 즉각 조치 사항 |\n"
                "| :--- | :--- | :--- | :--- |\n"
                "| **pH** | 5.8 ~ 8.6 | < 6.2 또는 > 8.3 | 폭기조 포기량 조절 및 유입수 이상 확인 |\n"
                "| **BOD** | 5.0 mg/L | 3.5 mg/L 초과 | 호기조 송풍량 증대, 내부반송 점검 |\n"
                "| **TOC** | 15.0 mg/L | 10.0 mg/L 초과 | 생물학적 난분해성 물질 유입 차단 |\n"
                "| **SS** | 10.0 mg/L | 7.0 mg/L 초과 | 2차 침전조 스컴 스키머 및 PAC 주입량 상향 |\n"
                "| **T-N** | 20.0 mg/L | 12.0 mg/L 초과 | 무산소조 C/N비 확보 및 내부반송 180% 상향 |\n"
                "| **T-P** | 0.20 mg/L | 0.08 mg/L 초과 | IPR 염화제이철 + 종침 PAC 즉시 20% 증량 |"
            )
            
        # 기본 통합 안내
        else:
            return (
                f"💡 **[단월 AI 통합 공정 진단 결과: '{user_query}']**\n\n"
                "단월 공공하수처리시설(본장 KNR+IPR) 및 소규모 6개소의 정상 운전 상태를 기준으로 분석하였습니다.\n\n"
                "1. **권장 운전점**: 유입 부하에 맞춰 호기조 DO 1.8~2.2 mg/L, C/N비 4.0 이상, IPR 질산액 반송 150~200% 유지를 권고합니다.\n"
                "2. **약품 투입**: 총인 농도 안정화를 위해 IPR 급속혼화지에 염화제이철, 2차 침전조 전단에 PAC을 정량 연동하십시오.\n"
                "3. **세부 문의 가능 주제**: 본장 KNR/IPR 제어, 3대 약품(염철/PAC/폴리머), 소규모 6개소(산음/삼가리/진목/몰운/단월마을/당의) 운전, TMS 수질 예측 및 비상 조치 등"
            )
    ...
    quick_q = None
    if chip_c1.button("📌 본장 KNR 질소(T-N) 제어법", use_container_width=True): quick_q = "단월 본장 KNR 질소(T-N) 고도처리 제어 가이드는?"
    if chip_c2.button("🧪 염철·PAC·폴리머 3대 약품", use_container_width=True): quick_q = "단월 본장 3대 약품(염화제이철, PAC, 폴리머) 투입 위치와 기준은?"
