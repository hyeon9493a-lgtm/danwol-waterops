...
# -------------------------------------------------------------
    ...
    with tab_proc_input:
        ...
        if btn_do_sync:
            ...
            if not df_master_fac.empty:
                ...
                st.success(f"✅ `{sel_proc_plant}` ({plant_method}) 공정 마스터 DB에 저장되었습니다!")
                st.dataframe(df_proc_synced, use_container_width=True)
            else:
                st.warning(f"⚠️ 1번 메뉴에 먼저 `{sel_proc_plant}` 관련 엑셀을 업로드해 주세요.")

        st.divider()
        st.markdown(f"##### 2️⃣ 금일 / 특정일자 유입 부하 조건 수동 등록 ({sel_proc_plant})")
        ...
        if st.button("💾 ⚡ [공정 운전 조건 & AI 권장값 마스터 DB 저장]", type="primary", use_container_width=True):
            ...
            st.success(f"✅ [{proc_date}] `{sel_proc_plant}` 공정 데이터가 마스터 DB에 저장되었습니다!")
    ...
    with tab_proc_guide:
        ...
        with st.expander(f"🎛️ [{sel_proc_plant}] 실시간 유입 부하 조건 시뮬레이션", expanded=False):
    ...
    with tab_proc_archive:
        ...
        if df_proc_all.empty:
            st.info(f"💡 아직 `{sel_proc_plant}`의 누적된 공정 제어 기록이 없습니다.")
        else:
...
# -------------------------------------------------------------
    ...
    if prompt := st.chat_input("질문을 입력하세요 (예: 삼가리 SBR 공정 T-N 수질이 높은데 어떻게 조절해야 해?)"):
        ...
        if "삼가리" in prompt or "SBR" in prompt:
            ...
            response_text = "단월처리시설 통합 관제 시스템 분석 결과, 현재 본장(1,700 ㎥/일) 및 소규모 6개소(산음/삼가리/진목/몰운/단월마을/당의) 고도처리 시스템은 법적 방류수질 기준을 안정적으로 충족하고 있습니다. 추가로 필요한 제어값이나 계산이 있으시면 말씀해 주세요."
...
3. 비상 구조용 삼각대, 송기마스크 및 구명줄 착용 상태 점검"""
    ...
    with tab_edu_write:
        ...
        with col_e1:
            ...
            edu_date = st.date_input("교육 실시 일자", datetime.date(2026, 8, 20), key="edu_date_in_v600")
            ...
            if uploaded_edu_files:
                ...
                if detected_subject:
                    ...
                    if st.button("⚡ [추출된 교안 내용으로 교육양식 자동 채우기]", type="primary", key="btn_auto_fill_v600"):
        ...
        with col_e2:
            ...
            with col_pad1:
                ...
                canvas_writer = st_canvas(stroke_width=2, stroke_color="#000000", background_color="#FFFFFF", height=70, width=150, drawing_mode="freedraw", key="canvas_edu_writer_v600")
            with col_pad2:
                st.caption("✍️ **결재자(시설장) 서명**")
                canvas_approver = st_canvas(stroke_width=2, stroke_color="#000000", background_color="#FFFFFF", height=70, width=150, drawing_mode="freedraw", key="canvas_edu_approver_v600")
            ...
            for num, d_dept, d_name in default_staff:
                ...
                with col_st2: s_dept = st.text_input(f"소속 #{num}", value=d_dept, key=f"edu_dept_{num}_v600", label_visibility="collapsed")
                with col_st3: s_name = st.text_input(f"성명 #{num}", value=d_name, key=f"edu_name_{num}_v600", label_visibility="collapsed")
        ...
        <!DOCTYPE html><html><head><meta charset="UTF-8"><style>
            ...
            <table class="main-form">
                ...
                <tr style="text-align:center; height:24px; font-weight:bold;">
                    <td class="col-header" rowspan="2">교&nbsp;&nbsp;&nbsp;&nbsp;육<br><br>인&nbspI encountered an error doing what you asked. Could you try again?
