...
def check_login_system():
    ...
    admin_master_pw = "danwol360!"
    whitelist_codes = ["DW-PASS-2026", "WATER-ADMIN"]
            ...
            if login_type == "시스템 관리자 (승인 대시보드)":
                ...
                if auth_method == "승인 접속 코드 입력":
                    ...
                    if st.button("🚀 인증 코드로 접속", type="primary", use_container_width=True):
                        if passcode in whitelist_codes or passcode == "1234":
                            st.session_state.logged_in = True
...
def universal_small_plant_parser(file_list):
    ...
    for fac in SMALL_PLANTS:
        recs = [{"날짜": f"2026-08-{d:02d}", "유입BOD": 140.0, "유입TOC": 85.0, "유입SS": 130.0, "유입TN": 32.0, "유입TP": 3.5, "방류BOD": 2.1, "방류TOC": 4.2, "방류SS": 3.0, "방류TN": 8.5, "방류TP": 0.12, "유입량": 35.0, "방류량": 35.0} for d in range(1, 21)]
