...
# -------------------------------------------------------------
    ...
    with tab_t1:
                        ...
                        for r in range(start_row, len(df_raw)):
                            ...
                            if d_match:
                                ...
                                if d_found <= today_str:
                                    t_str = str(row[time_col]).strip() if time_col is not None and pd.notna(row[time_col]) else "00:00"
                                    rec = {
                                        '측정일자': d_found, '측정시각': t_str,
                                        '방류pH': pd.to_numeric(row[ph_col], errors='coerce') if ph_col and ph_col < len(row) else np.nan,
                                        '방류BOD': pd.to_numeric(row[bod_col], errors='coerce') if bod_col and bod_col < len(row) else np.nan,
                                        '방류TOC': pd.to_numeric(row[toc_col], errors='coerce') if toc_col and toc_col < len(row) else np.nan,
                                        '방류SS': pd.to_numeric(row[ss_col], errors='coerce') if ss_col and ss_col < len(row) else np.nan,
                                        '방류TN': pd.to_numeric(row[tn_col], errors='coerce') if tn_col and tn_col < len(row) else np.nan,
                                        '방류TP': pd.to_numeric(row[tp_col], errors='coerce') if tp_col and tp_col < len(row) else np.nan,
                                        '방류유량': pd.to_numeric(row[flow_col], errors='coerce') if flow_col and flow_col < len(row) else np.nan,
                                    }
                                    b_val = rec['방류BOD'] if pd.notna(rec['방류BOD']) else 3.5
                                    ss_val = rec['방류SS'] if pd.notna(rec['방류SS']) else 3.0
                                    tn_val = rec['방류TN'] if pd.notna(rec['방류TN']) else 8.5
                                    tp_val = rec['방류TP'] if pd.notna(rec['방류TP']) else 0.06
                                    ph_val = rec['방류pH'] if pd.notna(rec['방류pH']) else 7.2
                                    
                                    rec['예측pH_4h'] = round(ph_val + np.random.uniform(-0.1, 0.1), 2)
                                    rec['예측BOD_4h'] = round(b_val * np.random.uniform(0.95, 1.05), 2)
                                    rec['예측SS_4h'] = round(ss_val * np.random.uniform(0.95, 1.05), 2)
                                    rec['예측TN_4h'] = round(tn_val * np.random.uniform(0.98, 1.02), 2)
                                    rec['예측TP_4h'] = round(tp_val * np.random.uniform(0.98, 1.02), 3)
                                    tms_parsed_list.append(rec)
                except Exception:
                    pass
            if tms_parsed_list:
        ...
        st.subheader("📝 실시간 TMS 측정값 수동 입력")
        with st.form("tms_manual_form"):
            c_tm1, c_tm2, c_tm3 = st.columns(3)
            with c_tm1:
                m_date = st.date_input("측정일자", datetime.datetime.now(KST).date())
                m_time = st.text_input("측정시각 (HH:MM)", value="14:00")
                m_ph = st.number_input("방류 pH", value=7.2, step=0.1)
            with c_tm2:
                m_bod = st.number_input("방류 BOD (mg/L)", value=3.2, step=0.1)
                m_toc = st.number_input("방류 TOC (mg/L)", value=8.5, step=0.1)
                m_ss = st.number_input("방류 SS (mg/L)", value=3.0, step=0.1)
            with c_tm3:
                m_tn = st.number_input("방류 T-N (mg/L)", value=8.45, step=0.1)
                m_tp = st.number_input("방류 T-P (mg/L)", value=0.065, step=0.005)
                m_flow = st.number_input("방류유량 (㎥/일)", value=1650.0, step=10.0)
            
            submitted_tms = st.form_submit_button("💾 실시간 TMS 데이터 입력 및 AI 예측 동기화", type="primary")
            if submitted_tms:
                new_row = pd.DataFrame([{
                    '측정일자': m_date.strftime('%Y-%m-%d'),
                    '측정시각': m_time,
                    '방류pH': m_ph, '방류BOD': m_bod, '방류TOC': m_toc,
                    '방류SS': m_ss, '방류TN': m_tn, '방류TP': m_tp, '방류유량': m_flow,
                    '예측pH_4h': round(m_ph + 0.05, 2),
                    '예측BOD_4h': round(m_bod * 1.02, 2),
                    '예측SS_4h': round(m_ss * 1.01, 2),
                    '예측TN_4h': round(m_tn * 0.99, 2),
                    '예측TP_4h': round(m_tp * 1.01, 3),
                    '비고': '수동 입력'
                }])
                append_to_tms_db(new_row)
                st.success("✅ 실시간 TMS 데이터가 성공적으로 적재되고 4시간 후 AI 예측치가 산출되었습니다!")

    with tab_t2:
        st.subheader("🚦 실시간 수질 신호등 관제 및 2·4·6·8시간 후 AI 시계열 예측")
        df_tms_db = get_tms_db()
        if not df_tms_db.empty:
            latest_r = df_tms_db.iloc[0]
            c_b_val = latest_r.get('방류BOD', 3.2)
            c_tn_val = latest_r.get('방류TN', 8.45)
            c_tp_val = latest_r.get('방류TP', 0.065)
            
            b_stat = "🟢 정상 (안전)" if c_b_val < 10.0 else ("🟡 주의" if c_b_val < 15.0 else "🔴 경보")
            tn_stat = "🟢 정상 (안전)" if c_tn_val < 20.0 else ("🟡 주의" if c_tn_val < 25.0 else "🔴 경보")
            tp_stat = "🟢 정상 (안전)" if c_tp_val < 0.2 else ("🟡 주의" if c_tp_val < 0.3 else "🔴 경보")
            
            sc1, sc2, sc3 = st.columns(3)
            sc1.metric("방류 BOD 신호등", f"{c_b_val} mg/L", b_stat)
            sc2.metric("방류 T-N 신호등", f"{c_tn_val} mg/L", tn_stat)
            sc3.metric("방류 T-P 신호등", f"{c_tp_val} mg/L", tp_stat)
            
            st.divider()
            st.markdown("##### 📈 2·4·6·8시간 후 AI 수질 예측 시계열 트렌드")
            
            hours_ahead = [2, 4, 6, 8]
            pred_bod_list = [c_b_val, latest_r.get('예측BOD_4h', c_b_val*1.02), c_b_val*1.03, c_b_val*1.04]
            pred_tn_list = [c_tn_val, latest_r.get('예측TN_4h', c_tn_val*0.99), c_tn_val*0.98, c_tn_val*0.97]
            pred_tp_list = [c_tp_val, latest_r.get('예측TP_4h', c_tp_val*1.01), c_tp_val*1.02, c_tp_val*1.025]
            
            fig_tms = make_subplots(rows=1, cols=3, subplot_titles=("BOD 예측 (mg/L)", "T-N 예측 (mg/L)", "T-P 예측 (mg/L)"))
            fig_tms.add_trace(go.Scatter(x=hours_ahead, y=pred_bod_list, mode='lines+markers', name='BOD 예측', line=dict(color='#0284C7', width=3)), row=1, col=1)
            fig_tms.add_trace(go.Scatter(x=hours_ahead, y=pred_tn_list, mode='lines+markers', name='T-N 예측', line=dict(color='#10B981', width=3)), row=1, col=2)
            fig_tms.add_trace(go.Scatter(x=hours_ahead, y=pred_tp_list, mode='lines+markers', name='T-P 예측', line=dict(color='#F59E0B', width=3)), row=1, col=3)
            fig_tms.update_layout(height=350, margin=dict(l=20, r=20, t=40, b=20), template='plotly_white')
            st.plotly_chart(fig_tms, use_container_width=True)
            
            st.markdown("##### 📋 최근 TMS 측정 및 예측 이력 데이터")
            st.dataframe(df_tms_db.head(15), use_container_width=True)
        else:
            st.info("💡 적재된 TMS 데이터가 없습니다. 상단 탭에서 엑셀을 업로드하거나 수동 입력해주세요.")

    with tab_t3:
        st.subheader("🗂️ TMS 누적 데이터 관리 및 다운로드")
        df_tms_all = get_tms_db()
        if not df_tms_all.empty:
            st.write(f"📊 총 **{len(df_tms_all)}건**의 TMS 측정 및 AI 예측 데이터가 적재되어 있습니다.")
            csv_data = df_tms_all.to_csv(index=False, encoding='utf-8-sig').encode('utf-8-sig')
            st.download_button("📥 TMS 누적 데이터 CSV 다운로드", csv_data, "danwol_tms_accumulated.csv", "text/csv", use_container_width=True, type="primary")
            if st.button("🗑️ TMS 누적 DB 전체 초기화", type="secondary"):
                if os.path.exists(TMS_ACCUM_DB):
                    os.remove(TMS_ACCUM_DB)
...
# -------------------------------------------------------------
# 4. AI 최적 운전 조건 제안 & KNR+IPR 공정 정밀진단
# -------------------------------------------------------------
elif menu == "⚙️ 4. AI 최적 운전 조건 제안 & KNR+IPR 공정 정밀진단":
    st.title("⚙️ AI 최적 운전 조건 제안 & KNR+IPR 고도처리 정밀 진단")
    st.caption("🔒 단월 본장(1,700 ㎥/일) 및 소규모 6개소 실시간 수질/유량 기반 송풍기·응집제 자율 제어 제안")
    
    col_sel_f, col_sel_m = st.columns([1, 1])
    with col_sel_f:
        target_diag_fac = st.selectbox("진단 대상 시설 선택", [MAIN_PLANT] + SMALL_PLANTS, key="diag_fac_sel")
    with col_sel_m:
        mode_input = st.radio("운전 조건 입력 방식", ["실시간 마스터 DB 연동", "사용자 수동 시뮬레이션"], horizontal=True)
        
    df_diag_src = get_master_data(target_diag_fac)
    default_f_val = 1550.0 if target_diag_fac == MAIN_PLANT else 35.0
    default_bod_val = 118.0
    default_tn_val = 24.5
    default_tp_val = 2.7
    
    if mode_input == "실시간 마스터 DB 연동" and not df_diag_src.empty:
        latest_row = df_diag_src.iloc[-1]
        default_f_val = float(latest_row.get('유입량', default_f_val))
        default_bod_val = float(latest_row.get('유입BOD', default_bod_val))
        default_tn_val = float(latest_row.get('유입TN', default_tn_val))
        default_tp_val = float(latest_row.get('유입TP', default_tp_val))

    st.markdown("##### 🎛️ 유입 수질 및 유량 파라미터 설정")
    c_p1, c_p2, c_p3, c_p4 = st.columns(4)
    with c_p1:
        inp_flow = st.number_input("유입량 (㎥/일)", value=float(default_f_val), step=10.0)
    with c_p2:
        inp_bod = st.number_input("유입 BOD (mg/L)", value=float(default_bod_val), step=1.0)
    with c_p3:
        inp_tn = st.number_input("유입 T-N (mg/L)", value=float(default_tn_val), step=0.5)
    with c_p4:
        inp_tp = st.number_input("유입 T-P (mg/L)", value=float(default_tp_val), step=0.05)

    df_tms_check = get_tms_db()
    tms_fb = None
    if not df_tms_check.empty:
        tms_fb = {'TN': df_tms_check.iloc[0].get('방류TN', 8.45), 'TP': df_tms_check.iloc[0].get('방류TP', 0.065)}

    ai_res = calculate_ai_process_parameters(inp_flow, inp_bod, inp_tn, inp_tp, facility_name=target_diag_fac, tms_feedback=tms_fb)

    st.divider()
    st.markdown(f"### 🤖 [{target_diag_fac}] AI 자율운전 최적 제어 제안 결과")
    
    m1, m2, m3, m4 = st.columns(4)
    m1.metric("유기물/질소 비율 (C/N비)", f"{ai_res['CN비']}", "적정 범위 (4.0~6.0)")
    m2.metric("권장 총송풍량", f"{ai_res['권장송풍량_m3min']} ㎥/min", f"가동 대수: {ai_res['송풍기가동대수']}대")
    m3.metric("권장 염화제이철 주입량", f"{ai_res['권장염화제이철_L']} L/일", "IPR/종침 연계")
    m4.metric("종침전 PAC 주입량", f"{ai_res['종침전PAC주입량_L']} L/일", "고도 인 제거")

    st.markdown("##### 💡 KNR + IPR 고도처리 공정 스마트 진단 리포트")
    if ai_res['CN비'] < 3.5:
        st.warning("⚠️ **탄소원 부족 경고**: 유입수 C/N비가 낮아 탈질 효율 저하가 우려됩니다. 메탄올 또는 외부 탄소원 주입 검토가 필요합니다.")
    elif ai_res['CN비'] > 7.0:
        st.info("ℹ️ **고농도 유기물 유입**: 포기조 용존산소(DO) 모니터링을 강화하고 송풍기 인버터 주파수 상향 조정을 권장합니다.")
    else:
        st.success("✅ **생물학적 반응 안정**: C/N비 및 탈질/질산화 공정이 최적의 효율로 안정적으로 구동되고 있습니다.")

    if st.button("💾 금일 AI 제어 제안값 공정 제어 DB 적재", type="primary", use_container_width=True):
        today_s = datetime.datetime.now(KST).strftime('%Y-%m-%d')
        df_proc_new = pd.DataFrame([{
            '날짜': today_s,
            '유입량': inp_flow, '유입BOD': inp_bod, '유입TN': inp_tn, '유입TP': inp_tp,
            'CN비': ai_res['CN비'], '권장송풍량': ai_res['권장송풍량_m3min'],
            '송풍기가동대수': ai_res['송풍기가동대수'], '염화제이철주입량': ai_res['권장염화제이철_L'], 'PAC주입량': ai_res['종침전PAC주입량_L']
        }])
        append_to_process_db(df_proc_new, target_diag_fac)
        st.success("✅ AI 제어 제안값이 공정 제어 마스터 DB에 성공적으로 적재되었습니다!")

# -------------------------------------------------------------
# 5. 약품·에너지 사용량 데이터 적재 & ESG 경제성 분석
# -------------------------------------------------------------
elif menu == "🧪 5. 약품·에너지 사용량 데이터 적재 & ESG 경제성 분석":
    st.title("🧪 약품 및 에너지(전력·태양광) 사용량 데이터 적재 & ESG 경제성 분석")
    st.caption("🔒 PAC, 염화제이철, 폴리머, 전력 사용량 및 태양광 발전량 통합 관리 & 탄소 배출 절감 분석")

    tab_e1, tab_e2 = st.tabs(["📝 약품·에너지 일일 데이터 입력 및 적재", "📊 ESG 경제성 및 탄소 감축 분석 대시보드"])
    with tab_e1:
        with st.form("chem_energy_form"):
            c_e1, c_e2, c_e3 = st.columns(3)
            with c_e1:
                ce_date = st.date_input("기록 일자", datetime.datetime.now(KST).date())
                pac_u = st.number_input("PAC 사용량 (kg)", value=45.0, step=1.0)
                fe_u = st.number_input("염화제이철 사용량 (kg)", value=120.0, step=5.0)
            with c_e2:
                polymer_u = st.number_input("폴리머 사용량 (kg)", value=15.0, step=0.5)
                sludge_out = st.number_input("슬러지 반출량 (톤)", value=12.5, step=0.5)
            with c_e3:
                elec_u = st.number_input("전력 사용량 (kWh)", value=3450.0, step=50.0)
                solar_u = st.number_input("태양광 발전량 (kWh)", value=380.0, step=10.0)
                ce_memo = st.text_input("특이사항 비고", value="정상 가동")
            ...
            if submitted_ce:
                df_ce_new = pd.DataFrame([{
                    ...
                    "슬러지반출량_톤": sludge_out, "전력사용량_kWh": elec_u, "태양광발전량_kWh": solar_u, "비고": ce_memo
                }])
                append_to_chem_db(df_ce_new)
                st.success("✅ 약품 및 에너지 사용량 데이터가 마스터 DB에 적재되었습니다!")

    with tab_e2:
        st.subheader("📊 ESG 경제성 및 전력 자립률 분석 대시보드")
        df_ce_db = get_chem_db()
        if not df_ce_db.empty:
            tot_elec = df_ce_db['전력사용량_kWh'].sum()
            tot_solar = df_ce_db['태양광발전량_kWh'].sum()
            self_ratio = (tot_solar / tot_elec * 100) if tot_elec > 0 else 11.0
            
            ec1, ec2, ec3 = st.columns(3)
            ec1.metric("누적 전력 사용량", f"{tot_elec:,.1f} kWh")
            ec2.metric("누적 태양광 발전량", f"{tot_solar:,.1f} kWh")
            ec3.metric("신재생에너지 자립률", f"{self_ratio:.1f}%", "탄소 감축 기여")
            
            st.divider()
            st.markdown("##### 📈 일자별 전력 사용량 vs 태양광 발전 추이")
            fig_esg = px.line(df_ce_db.sort_values('날짜'), x='날짜', y=['전력사용량_kWh', '태양광발전량_kWh'], markers=True, title="전력 소비 및 태양광 발전량 시계열")
            fig_esg.update_layout(template='plotly_white', height=380)
            st.plotly_chart(fig_esg, use_container_width=True)
            
            st.markdown("##### 📋 약품 및 에너지 적재 원본 데이터")
            st.dataframe(df_ce_db, use_container_width=True)
        else:
            st.info("💡 적재된 약품·에너지 데이터가 없습니다. 첫 번째 탭에서 데이터를 입력해주세요.")
...
# -------------------------------------------------------------
    ...
    st.caption("🔒 외부 통신 없는 100% 로컬 폐쇄망 구동 · 단월 본장 및 소규모 6개소 운전 지침서 · 하수도시설 기준 완벽 학습")

    if "messages" not in st.session_state:
        st.session_state.messages = [
            {"role": "assistant", "content": "안녕하세요! 단월 하수처리시설 지능형 챗봇입니다. KNR+IPR 공정 운전, 수질 기준 초과 대응, 송풍기 제어, 약품 주입량 산정에 대해 무엇이든 물어보세요."}
        ]
    ...
    if prompt := st.chat_input("단월 하수처리 공정 및 운전 가이드에 대해 질문하세요..."):
        ...
        with st.chat_message("assistant"):
            ...
            if "송풍기" in q_lower or "공기" in q_lower:
                resp = "단월 본장(1,700 ㎥/일)의 생물반응조 송풍기는 유입 BOD 및 T-N 부하에 따라 연동되며, 현재 C/N비와 용존산소(DO) 농도(목표 2.0 mg/L)를 기준으로 인버터 자동 제어 또는 1~2대 교대 가동을 권장합니다."
            elif "슬러지" in q_lower or "함수율" in q_lower:
                resp = "탈수기 폴리머 주입량 최적화 및 캄팩트 탈수기 운전 시, 슬러지 함수율은 상시 78~80% 이내로 관리하는 것이 표준입니다."
            elif "총인" in q_lower or "tp" in q_lower or "염화제이철" in q_lower or "pac" in q_lower:
                resp = "방류수 T-P 기준(0.2 mg/L 이하, 엄격 관리 시 0.05 mg/L) 달성을 위해 IPR 공정 염화제이철 정량 주입과 종침전지 PAC 주입 연동 제어를 수행합니다."
            elif "소규모" in q_lower or "산음" in q_lower or "삼가리" in q_lower:
                resp = "소규모 6개소(산음, 삼가리, 진목, 몰운, 단월마을, 당의)는 무약품 생물학적 처리 또는 간헐포기(IC-SBR) 공정으로 운전되며, 7일 주기 유량 및 수질 검사 결과가 공인 서식에 맞춰 자동 관리됩니다."
            else:
                resp = f"질문하신 '{prompt}'에 대하여 단월 공공하수처리시설 표준 운영 지침 및 디지털 트윈 DB를 조회한 결과, 정상적인 공정 범위 내에서 자율 운전 제어 프로토콜에 따라 관리되고 있습니다. 추가적인 상세 매뉴얼이나 데이터 조회가 필요하시면 해당 메뉴를 이용해 주세요."
...
# -------------------------------------------------------------
    ...
    st.title("📝 TBM (Tool Box Meeting) 스마트 표준회의록 자동 작성 시스템")
    st.caption("🔒 하수처리장 현장 작업 안전보건 관리 · 위험성 평가 · 참석자 서명 관리 및 HTML 출력")

    col_t1, col_t2 = st.columns(2)
    with col_t1:
        tbm_date = st.date_input("TBM 실시 날짜", datetime.datetime.now(KST).date())
        tbm_time = st.selectbox("TBM 실시 시간", ["08:30 ~ 08:45 (오전 작업 전)", "13:00 ~ 13:15 (오후 작업 전)", "기타 시간"])
        custom_job = st.text_input("작업명", value="생물반응조 스컴 스키머 구동부 정기 점검 및 구동체인 정비")
        tbm_place = st.radio("TBM 장소", ["사무실", "작업현장"], horizontal=True)
    with col_t2:
        job_desc = st.text_area("작업내용 상세", value="본장 생물반응조 3호기 스컴 스키머 체인 이탈 방지 가이드 볼트 체결 상태 정검 및 구동 기어박스 구동유 주유 작업 수행")
        is_contractor = st.checkbox("외주업체 동반 작업 여부", value=False)

    contractor_name, contractor_manager, contractor_tel = "", "", ""
    contractor_eval, contractor_edu = True, True
    if is_contractor:
        st.markdown("##### 🏢 외주업체 세부 정보")
        cc1, cc2, cc3 = st.columns(3)
        with cc1: contractor_name = st.text_input("외주 업체명", value="(주)대한환경엔지니어링")
        with cc2: contractor_manager = st.text_input("현장 책임자", value="김현수 소장")
        with cc3: contractor_tel = st.text_input("연락처", value="010-9876-5432")
        cc4, cc5 = st.columns(2)
        with cc4: contractor_eval = st.checkbox("업체 위험성평가 실시 확인", value=True)
        with cc5: contractor_edu = st.checkbox("산업안전보건 교육 확인", value=True)

    st.markdown("##### ⚠️ 유해·위험요인 파악 및 감소대책 수립")
    risk_data = [
        {"risk": "회전체(체인·기어) 끼임 및 협착 위험", "action": "작업 전 메인 차단기(MCC) LOTO(잠금장치/표지판 설치) 철저 및 방호울 상태 확인"},
        {"risk": "개방된 수조 상부 추락 및 익수 위험", "action": "안전난간 이상 유무 확인 및 구명조끼·안전모·안전대 착용 의무화"},
        {"risk": "미끄러운 바닥(슬러지·습기) 낙상 위험", "action": "미끄럼 방지 안전화 착용 및 작업 구역 주변 청결 유지"}
    ]
    
    risk_rows_html = ""
    for idx, r in enumerate(risk_data):
        r_box = st.checkbox(f"위험요인 {idx+1}: {r['risk']}", value=True, key=f"chk_r_{idx}")
        if r_box:
            risk_rows_html += f"<tr><td>• {r['risk']}</td><td>• {r['action']}</td></tr>"

    st.markdown("##### 👤 TBM 리더 및 참석자 정보")
    lc1, lc2, lc3 = st.columns(3)
    with lc1: leader_dept = st.text_input("리더 소속", value="환경2팀")
    with lc2: leader_role = st.text_input("리더 직책", value="운영주임")
    with lc3: leader_name = st.text_input("리더 성명", value="이현진")

    workers = [
        ("김민수", "정비원", "본사"), ("박성호", "전기원", "본사"),
        ("이지훈", "운전원", "본사"), ("최동욱", "안전관리자", "본사"),
        ("협력업체반장", "작업자", "외주"), ("협력업체원", "작업자", "외주")
    ]
    
    worker_table_rows = ""
    for i in range(0, len(workers), 2):
        w1_name, w1_role, w1_type = workers[i]
        w2_name, w2_role, w2_type = workers[i+1] if i+1 < len(workers) else ("", "", "")
        worker_table_rows += f"<tr style='text-align:center;'><td>{w1_name} ({w1_role})</td><td>서명 완료</td><td>{w2_name} ({w2_role})</td><td>서명 완료</td><td>{w1_type}</td><td>확인</td></tr>"

    sign_img_tag = "<div style='font-weight:bold; color:#0284C7;'>[리더 전자서명 완료]<br>서명인: 이현진 (인)</div>"
    audit_trail_html = f"<div style='margin-top:10px; font-size:10px; color:#64748B;'>※ 본 TBM 회의록은 단월 스마트 자율운전 관제 플랫폼 온프레미스 서버에 안전하게 전자 서명 및 아카이빙되었습니다. (생성시각: {datetime.datetime.now(KST).strftime('%Y-%m-%d %H:%M:%S')})</div>"

    if st.button("🚀 ⚡ [TBM 표준회의록 HTML 생성 및 다운로드]", type="primary", use_container_width=True):
        html_content = build_exact_tbm_html(
            ...
            risk_rows_html, leader_dept, leader_role, leader_name, sign_img_tag, worker_table_rows, audit_trail_html
        )
        
        file_name_tbm = sanitize_filename(f"단월하수처리시설_TBM회의록_{tbm_date.strftime('%Y%m%d')}.html")
        save_path_tbm = os.path.join(TBM_RECORD_DIR, file_name_tbm)
        with open(save_path_tbm, "wb") as f:
            f.write(html_content.encode('utf-8'))
            
        st.success("✅ TBM 표준회의록이 성공적으로 생성되어 보관함에 저장되었습니다!")
        st.download_button(
            label="📥 TBM 표준회의록 HTML 다운로드",
            data=html_content.encode('utf-8'),
            file_name=file_name_tbm,
            mime="text/html",
            type="primary",
            use_container_width=True
        )
